  ...
  return (
    <>
      {optionCardsData.map((card) => (
        // Add the 'key' prop here, using a unique identifier from your card object
        <Col key={card.key} xs={12} md={6} lg={3}>
          <Card
            className="border-0 shadow-sm option-card h-100" // Added h-100 for consistent height
            ...
            <Card.Body className="p-4 d-flex flex-column align-items-center justify-content-center text-center">
              <div
                ...
                style={{
                  width: "56px",
                  height: "56px",
                  backgroundColor: "rgba(var(--bs-primary-rgb), 0.1)", // Dynamic background color based on icon
                }}
              >
                {card.icon}
              </div>
              ...
              <p className="text-muted mb-0">
                {card.key === "fee" && optionData[card.key] === "0" 
                  ? "Free"
                  : optionData[card.key]
