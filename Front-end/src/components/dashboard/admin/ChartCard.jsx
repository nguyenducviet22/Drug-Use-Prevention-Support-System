...
const ChartCard = ({ title, type }) => {
  ...
  const generateBarData = () => ({
    labels: ['Students', 'College Students', 'Parents', 'Teachers', 'Others'],
    datasets: [
      {
  ...
  const generateDoughnutData = () => ({
    labels: ['Students (12-18)', 'College (18-25)', 'Parents (25-45)', 'Teachers (25-60)', 'Others'],
    datasets: [
      {
        data: [35, 25, 20, 15, 5],
        backgroundColor: [
  ...
  const renderChart = () => {
    switch (type) {
      ...
      case 'doughnut':
        return <Doughnut data={generateDoughnutData()} options={chartOptions} />
      default:
        return <Line data={generateLineData()} options={chartOptions} />
    }
  }
