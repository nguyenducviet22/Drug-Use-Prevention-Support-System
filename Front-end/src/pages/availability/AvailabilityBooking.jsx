...
const AvailabilityBooking = () => {
  ...
  const {
    loading: loadingConsultantAvailableSlots,
    error: errorConsultantAvailableSlots,
    get: getConsultantAvailableSlots,
  } = useFetch();
  const {
    loading: loadingConsultantScheduledSlots,
    error: errorConsultantScheduledSlots,
    get: getConsultantScheduledSlots,
  } = useFetch();
  const {
    loading: loadingConsultantCancelledSlots,
    error: errorConsultantCancelledSlots,
    get: getConsultantCancelledSlots,
  } = useFetch();
  const {
    loading: loadingConsultantConfirmedSlots,
    error: errorConsultantConfirmedSlots,
    get: getConsultantConfirmedSlots,
  } = useFetch();
  const {
    loading: loadingNewAvailabilities,
    error: errorNewAvailabilities,
    post: postNewAvailabilities,
  } = useFetch();
  const {
    loading: loadingCancelAvailability,
    error: errorCancelAvailability,
    put: cancelScheduledSlots,
  } = useFetch();
  ...
  const transformToCalendarEvents = (rawData, consultantID, type) => {
    ...
    return rawData.map((timeString) => {
      ...
      if (type === "available") {
        eventTitle = isAvailableForBooking
          ? t("calendarSection.eventTitles.available")
          : t("calendarSection.eventTitles.pastUnavailable");
        backgroundColor = isAvailableForBooking ? "#28a745" : "#6c757d";
        borderColor = isAvailableForBooking ? "#28a745" : "#6c757d";
      } else if (type === "scheduled") {
        eventTitle = isPastSlot
          ? t("calendarSection.eventTitles.pastScheduled")
          : t("calendarSection.eventTitles.scheduled");
  ...
  const fetchConsultantSlots = async (consultantID, fromDate, toDate) => {
    if (!consultantID || !fromDate || !toDate) {
      console.warn(
        "Cannot fetch availabilities: missing consultantID, fromDate, or toDate."
      );
      return;
    }
    try {
      ...
      const transformedCancelledEvents = transformToCalendarEvents(
        cancelledRawSlots,
        consultantID,
        "cancelled"
      );
      const cancelledTimes = new Set(
        transformedCancelledEvents.map(
          (event) => event.extendedProps.originalTime
        )
      );
      ...
      const filteredConfirmedRawSlots = confirmedRawSlots.filter(
        (confirmedTime) => {
          return !cancelledTimes.has(confirmedTime);
        }
      );
      const transformedConfirmedEvents = transformToCalendarEvents(
        filteredConfirmedRawSlots,
        consultantID,
        "confirmed"
      );
      const confirmedTimes = new Set(
        transformedConfirmedEvents.map(
          (event) => event.extendedProps.originalTime
        )
      );
      ...
      // Filter out any scheduled slots that are also marked as cancelled
      const filteredScheduledRawSlots = scheduledRawSlots.filter(
        (scheduledTime) => {
          return !confirmedTimes.has(scheduledTime);
        }
      );
      const transformedScheduledEvents = transformToCalendarEvents(
        filteredScheduledRawSlots,
        consultantID,
        "scheduled"
      );
      const scheduledTimes = new Set(
        transformedScheduledEvents.map(
          (event) => event.extendedProps.originalTime
        )
      );
      ...
      const combinedUnavailableTimes = new Set([
        ...cancelledTimes,
        ...confirmedTimes,
        ...scheduledTimes,
      ]);

      const filteredAvailableRawSlots = availableRawSlots.filter(
        (availableTime) => {
          return !combinedUnavailableTimes.has(availableTime);
        }
      );
      const transformedAvailableEvents = transformToCalendarEvents(
        filteredAvailableRawSlots,
        consultantID,
        "available"
      );
  ...
  const handleEventClick = (clickInfo) => {
    ...
    if (
      (event.extendedProps.type === "scheduled" ||
    ...
    if (
      ...
      const eventIndexInCalendar = updatedCalendarEvents.findIndex(
        (evt) => evt.id === event.id
      );

      if (isSelected) {
        updatedSelectedSlots = selectedTimeSlots.filter(
          (slot) => slot.id !== clickedSlot.id
        );
  ...
  const handleConfirmBooking = async () => {
    try {
      ...
      const availabilityDateTimes = slotsToBook.map((slot) =>
      ...
      const response = await postNewAvailabilities(
        availabilitiesData,
        {},
      ...
      if (response) {
        ...
        toast.error(
          t("toasts.bookingError", {
            message: response?.message || t("toasts.unknownBookingError"),
          })
        );
      }
    } catch (error) {
  ...
  const handleConfirmCancellation = async () => {
    ...
    try {
      ...
      const response = await cancelScheduledSlots(
        requestBody,
        {},
        "http://localhost:8080/api/availability/CANCELLED"
      );
      ...
      if (response) {
        ...
        toast.error(
          t("toasts.cancellationError", {
            message: response?.message || t("toasts.unknownCancellationError"),
          })
        );
      }
    } catch (error) {
  ...
  const calendarOptions = {
    ...
    eventMouseEnter: (info) => {
      ...
      if (
        (type === "available" && !isPast) ||
        (type === "scheduled" && !isPast) ||
        (type === "confirmed" && !isPast)
      ) {
    ...
    eventDidMount: (info) => {
      const { type, isPast } = info.event.extendedProps;
      const isCurrentlySelected = selectedTimeSlots.some(
        (slot) => slot.id === info.event.id
      );
              ...
              <Card.Body>
                <div className="d-flex align-items-center justify-content-between mb-4">
                  <h4 className="mb-0 text-primary fw-bold">
                    {t("calendarSection.availableTimeSlotsTitle")}
                  </h4>
                </div>
                {consultantID ? (
                  ...
                  <div className="calendar-placeholder">
                    <div className="text-center py-5">
                      ...
                      <h5 className="text-muted">
                        {t("calendarSection.placeholderLoading")}
                      </h5>
                    </div>
                  </div>
                )}
              ...
              <Card className="appointment-info-card mt-4">
                <Card.Body>
                    ...
                    {selectedTimeSlots.map((slot, index) => (
                      <div key={slot.id} className="row mb-2">
                        <div className="col-md-6">
                          <strong>{t("selectedSlotsSection.dateLabel")}</strong>{" "}
                          {slot.formattedDate}
                        </div>
                        <div className="col-md-6">
                          <strong>{t("selectedSlotsSection.timeLabel")}</strong>{" "}
                          {slot.formattedTime}
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="text-center">
                    ...
                    >
                      {t("selectedSlotsSection.submitButton", {
        ...
        <Modal.Body className="confirmation-modal">
            ...
            {selectedTimeSlots.map((slot, index) => (
              <div key={index} className="detail-row mb-2">
                <span className="detail-label">
                  {t("confirmationModal.slotLabel", { index: index + 1 })}
                </span>
          ...
          <div className="text-center mt-4">
            <Button
              ...
              disabled={loadingNewAvailabilities}
            >
              {loadingNewAvailabilities
                ? t("confirmationModal.confirmingButton")
        ...
        <Modal.Body className="confirmation-modal">
          ...
          <div className="text-center mt-4">
            <Button
              ...
              disabled={loadingCancelAvailability}
            >
              {loadingCancelAvailability
                ? t("cancellationModal.cancellingButton")
                : t("cancellationModal.confirmCancellationButton")}
            </Button>
            <Button
              variant="secondary"
              size="lg"
